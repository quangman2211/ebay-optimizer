...
class OrderRepository(CRUDBase[Order, OrderCreate, OrderUpdate]):
    ...
    def get_by_order_number(self, db: Session, *, order_number: str, user_id: int = None) -> Optional[Order]:
    ...
    def get_by_order_id(self, db: Session, *, order_id: str, user_id: int) -> Optional[Order]:
        """Get order by eBay order ID"""
        return db.query(Order).filter(
            and_(Order.order_number == order_id, Order.user_id == user_id)
        ).first()

    def create_from_dict(self, db: Session, *, obj_in: Dict[str, Any]) -> Order:
    ...
    def count_by_account(self, db: Session, *, account_id: int, user_id: int) -> int:
        ...
        return db.query(Order).filter(
            and_(Order.account_id == account_id, Order.user_id == user_id)
        ).count()

    def get_latest_by_account(self, db: Session, *, account_id: int, user_id: int) -> Optional[Order]:
        """Get latest order by account"""
        return db.query(Order).filter(
            and_(Order.account_id == account_id, Order.user_id == user_id)
        ).order_by(desc(Order.updated_at)).first()
