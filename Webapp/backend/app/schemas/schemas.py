...
class CSVImportRequest(BaseModel):
    ...
    import_type: str = Field(..., pattern="^(orders|listings)$")
    estimated_records: Optional[int] = 0
...
class OrderCSVRecord(BaseModel):
    ...
    sold_for: float = Field(default=0.0, ge=0)
...
class ListingCSVRecord(BaseModel):
    item_number: str = Field(..., min_length=1)
    title: str = Field(..., min_length=1)
    custom_label: Optional[str] = None
    available_quantity: int = Field(default=0, ge=0)
    current_price: float = Field(..., gt=0)
    sold_quantity: Optional[int] = Field(default=0, ge=0)
    watchers: Optional[int] = Field(default=0, ge=0)
